<div id="container">
    ...
    //Tsükel HTML värvidest
    for ($i = 1; $i < count($colors_masiivid); $i++) {
        echo "<span style='color:$colors_masiivid[$i]'>$colors_masiivid[$i]<br></span>";
        echo "<div style='background-color: $colors_masiivid[$i]'>$colors_masiivid[$i]</div>";

    }
    ?>
    <div id="yl2" class="item">
        <h2>Ülesanne 2.</h2>
        PHP - masiivid
        Autod massiiviandmed võta lehelt -
        https://www.metshein.com/unit/php-massiivid-ulesanne-5/
        <br>* Kuva kõik autod loeteluga
    ...
    $total_count = count($autod);
    echo "<br>Autode arv  - $total_count";


    echo "<h2>Kas massiiv on ühepikkused või ei?</h2>";
    if (count($autod) == count($VIN_Kode_Autod)) {
        echo "Massiiv on ühepikkused";
    }
    else {
        echo "Massiiv ei ole ühespikkused";
    }


    echo "<h2>Kui palju toyotaid ja audiid</h2>";
    $toyota_arv = 0;
    $audi_arv = 0;
    foreach ($autod as $auto) {
        if ($auto === "Toyota") {
    ...
    echo "<br>Toyotaid: $toyota_arv";
    echo "<br>Audiid: $audi_arv";


    echo "<h2>VIN koodid,  mille märkide arv on väiksem kui 17</h2>";
    $VinKoded = "";
    foreach ($VIN_Kode_Autod as $VinKode) {
        if (strlen($VinKode) < 17) {
            echo "<br>$VinKode";
        }
    }
    ?>
</div>
<div id="yl3" class="item">
    <h2>Ülesanne 3</h2>
...
for ($p = 0; $p < count($pildiArray); $p++) {
    echo "<img src='Content/img/$pildiArray[$p]' width='100px' alt='' style='margin-right: 10px'>";
}
?>
